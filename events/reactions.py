            ...
            if str(payload.emoji) == res["emoji"]:
                ...
                if len(reactions) > 0:
                    ...
                    if not check:
                        ...
                        if mes.reference:
                            embed.description = f"{embed.description}\n<:righty:1206341364363559003> [replying to {mes.reference.resolved.author}]({mes.reference.resolved.jump_url})"
                            ...
                            if role_id:
                                if role := channel.guild.get_role(role_id):
                                    if role not in mes.author.roles:
                                        await mes.author.add_roles(
                                            role, reason=f"user is in the starboard"
                                        )
