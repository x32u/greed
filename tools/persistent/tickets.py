...
class DeleteTicket(discord.ui.Button):
    ...
    async def make_transcript(self, interaction: discord.Interaction, c: discord.TextChannel):
        user_id = await interaction.client.db.fetchval("""SELECT user_id FROM opened_tickets WHERE channel_id = $1""", c.id)
        user = c.guild.get_member(user_id)
        author = TicketAuthor(**{'id': str(user.name), 'name': user.display_name, 'discriminator': str(user.discriminator), 'avatar_url': str(user.display_avatar), 'mod': False})
