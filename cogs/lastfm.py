...
class Lastfm(Cog):
    ...
    async def lastfm_replacement(self, user: str, params: str) -> str:
        ...
        params = (
            ...
            .replace("{lastfm.color}", "0xB1AAD8")
            .replace("{lastfm.emoji}", "<:lavishfm:1204627517047177297>")
        )
    ...
    async def lf_chart(self, ctx: PretendContext, *, user: Member = Author): 
        """
        Get a collage of a member's top albums
        """

        username = await self.bot.db.fetchval("SELECT username FROM lastfm WHERE user_id = $1", user.id)

        if not username:
            return await ctx.send_error(f"{user.mention} does not have a linked **Last.FM** accont")
        
        x = await self.bot.pretend.lastfm_chart(username)
        image = BytesIO(await self.bot.session.get_bytes(x.image_url))

        return await ctx.reply(
            f"**{user.name}'s** top albums", 
            file=File(image, filename="chart.png")
        )
