...
from tools.converters import NewRoleConverter, HexColor, Alias
...
class Config(Cog):
    ...
    @command(name='autopfp', brief='manageguild')
    @has_guild_permissions(manage_channels=True)
    async def autopfp(self, ctx: PretendContext, *, channel: TextChannel = None):
        if channel is None:
            await self.bot.db.execute("""DELETE FROM autopfp WHERE guild_id = $1""", ctx.guild.id)
            return await ctx.send_success(f"Disabled autopfp")

        if not channel.is_nsfw():
            return await ctx.send("The channel must be NSFW.")

        await self.bot.db.execute("""INSERT INTO autopfp (guild_id, channel_id) VALUES($1,$2)
                                     ON CONFLICT(guild_id) DO UPDATE SET channel_id = excluded.channel_id""", ctx.guild.id, channel.id)
        return await ctx.send_success(f"Enabled autopfp")

    async def get_random_pfps(self) -> str:
        user = choice(self.bot.users)
        if user.avatar:
    ...
    async def autopfp_loop(self):
        async with self.locks['autopfp']:
            data = await self.bot.db.fetch("""SELECT guild_id, channel_id FROM autopfp""")
            for row in data:
                pfp = await self.get_random_pfps()
                if pfp:
                    async with self.locks['autopfp_send']:
                        channel = self.bot.get_channel(int(row["channel_id"]))
                        if channel:
                            if not channel.guild.chunked:
                                await channel.guild.chunk(cache=True)
                            await channel.send(embed=Embed(title='New pfp', url=pfp).set_image(url=pfp).set_footer(text="Sent from greed ^^"))
    ...
    async def alias_add(self, ctx: PretendContext, *, args: Alias):
        await self.bot.db.execute("""INSERT INTO aliases (guild_id,alias,command_name) VALUES($1,$2,$3) ON CONFLICT (guild_id, alias) DO UPDATE SET command_name = excluded.command_name""", ctx.guild.id, args[1],args[0].qualified_name)
        return await ctx.send_success(f"created the alias **{args[1]}** for **{args[0]}**")

    @alias.command(name = 'remove', aliases = ['del','d','rem','r'], brief = 'manage_guild')
    @has_guild_permissions(manage_guild = True)
    async def alias_remove(self, ctx: PretendContext, *, args: Alias):
        command, alias = args
        await self.bot.db.execute("""DELETE FROM aliases WHERE guild_id = $1 AND alias = $2 AND command_name = $3""", ctx.guild.id, alias, command.qualified_name)
    ...
    async def alias_list(self, ctx: PretendContext):
        data = await self.bot.db.fetch('SELECT command_name, alias FROM aliases WHERE guild_id = $1', ctx.guild.id)
        if not data: return await ctx.send_warning("no aliases found")
        return await ctx.paginate([f"**{alias}** - `{command_name}`" for command_name, alias in data], f"Aliases ({len(data)})")
    ...
    @has_guild_permissions(manage_guild=True)
    async def starboard_reward(self, ctx: PretendContext, *, role: Role):
