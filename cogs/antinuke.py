...
class Antinuke(Cog):
    def __init__(self, bot: Pretend):
        ...
        self.thresholds = {}
        self.modules = ['channel delete','channel create','role giving','role delete','role create','kick','ban','edit role','mass mention','spammer','new accounts','bot add']
    ...
    async def enable_antinuke(self, ctx, threshold: int = None, punishment: str = 'ban'):
        return await asyncio.gather(*[self.do_enable(ctx, m, threshold, punishment) for m in self.modules])

    async def do_enable(self, ctx, m:str, threshold: int = None, punishment: str = 'ban'):
        if not await self.bot.db.fetchrow('SELECT * FROM antinuke_modules WHERE guild_id = $1 AND module = $1', ctx.guild.id, m):
            await self.bot.db.execute('INSERT INTO antinuke_modules VALUES($1,$2,$3,$4)', ctx.guild.id, m, punishment, threshold)
            return True

    @antinuke.command(name='enable', aliases = ['start'])
    @antinuke_configured()
    @admin_antinuke()
    async def an_enable(self, ctx, threshold: int = 1, punishment: typing.Optional[Punishment]='ban'):
        await self.enable_antinuke(ctx,threshold,punishment)
        return await ctx.send_success('Enabled **antinuke** with a threshold of `{threshold}` and the **punishment** is `{punishment}`')
