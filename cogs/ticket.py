...
class Ticket(Cog):
    ...
    async def cog_load(self):
        await self.bot.db.execute("""CREATE TABLE IF NOT EXISTS logs ( key TEXT NOT NULL, guild_id BIGINT NOT NULL, channel_id BIGINT NOT NULL, author JSONB NOT NULL DEFAULT '{}'::JSONB, logs JSONB NOT NULL DEFAULT '{}'::JSONB, PRIMARY KEY(key));""")

    async def make_transcript(self, c: TextChannel):
        user_id = await self.bot.db.fetchval("""SELECT user_id FROM opened_tickets WHERE channel_id = $1""", c.id)
        user = self.bot.get_user(user_id)
        if not user:
            return None
        author = {'id': str(user.id), 'name': user.name, 'discriminator': user.discriminator, 'avatar_url': str(user.display_avatar.url), 'mod': False}
